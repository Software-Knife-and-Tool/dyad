//  SPDX-FileCopyrightText: Copyright 2022 James M. Putnam (putnamjm.design@gmail.com)
//  SPDX-License-Identifier: MIT

//! mu fixnum type
use crate::{
    core::{
        classes::{Tag, Type},
        exception,
        exception::{Condition, Exception, Result},
        frame::Frame,
        mu::{Core as _, Mu},
    },
    types::{
        r#struct::Struct,
        symbol::{Core as _, Symbol},
    },
};

#[derive(Copy, Clone)]
#[allow(dead_code)]
pub enum Fixnum {
    Direct(u64),
}

impl Fixnum {
    // u64 to tag
    pub fn as_tag(fx: i64) -> Tag {
        // we're implicitly or'ing in the fixnum base tag type 0 here
        Tag::Fixnum(fx << 2)
    }

    // tag as i64
    pub fn as_i64(mu: &Mu, tag: Tag) -> i64 {
        match Tag::type_of(mu, tag) {
            Type::Fixnum => Tag::data(&tag, mu) as i64,
            _ => panic!("internal: fixnum type inconsistency"),
        }
    }
}

pub trait Core {
    fn write(_: &Mu, _: Tag, _: bool, _: Tag) -> Result<()>;
    fn view(_: &Mu, _: Tag) -> Tag;
}

impl Core for Fixnum {
    fn write(mu: &Mu, tag: Tag, _escape: bool, stream: Tag) -> Result<()> {
        mu.write_string(Self::as_i64(mu, tag).to_string(), stream)
    }

    fn view(mu: &Mu, fx: Tag) -> Tag {
        Struct::to_tag(mu, Symbol::keyword("fixnum"), vec![fx])
    }
}

pub trait MuFunction {
    fn mu_fxadd(_: &Mu, _: &mut Frame) -> exception::Result<()>;
    fn mu_fxsub(_: &Mu, _: &mut Frame) -> exception::Result<()>;
    fn mu_fxor(_: &Mu, _: &mut Frame) -> exception::Result<()>;
    fn mu_fxand(_: &Mu, _: &mut Frame) -> exception::Result<()>;
    fn mu_fxdiv(_: &Mu, _: &mut Frame) -> exception::Result<()>;
    fn mu_fxlt(_: &Mu, _: &mut Frame) -> exception::Result<()>;
    fn mu_fxmul(_: &Mu, _: &mut Frame) -> exception::Result<()>;
}

impl MuFunction for Fixnum {
    fn mu_fxadd(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let fx0 = fp.argv[0];
        let fx1 = fp.argv[1];

        match Tag::type_of(mu, fx0) {
            Type::Fixnum => match Tag::type_of(mu, fx1) {
                Type::Fixnum => {
                    fp.value = Self::as_tag(Fixnum::as_i64(mu, fx0) + Fixnum::as_i64(mu, fx1));
                    Ok(())
                }
                _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-add", fx1)),
            },
            _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-add", fx0)),
        }
    }

    fn mu_fxsub(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let fx0 = fp.argv[0];
        let fx1 = fp.argv[1];

        match Tag::type_of(mu, fx0) {
            Type::Fixnum => match Tag::type_of(mu, fx1) {
                Type::Fixnum => {
                    fp.value = Self::as_tag(Fixnum::as_i64(mu, fx0) - Fixnum::as_i64(mu, fx1));
                    Ok(())
                }
                _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-sub", fx1)),
            },
            _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-sub", fx0)),
        }
    }

    fn mu_fxmul(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let fx0 = fp.argv[0];
        let fx1 = fp.argv[1];

        match Tag::type_of(mu, fx0) {
            Type::Fixnum => match Tag::type_of(mu, fx1) {
                Type::Fixnum => {
                    fp.value = Self::as_tag(Fixnum::as_i64(mu, fx0) * Fixnum::as_i64(mu, fx1));
                    Ok(())
                }
                _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-mul", fx1)),
            },
            _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-mul", fx0)),
        }
    }

    fn mu_fxlt(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let fx0 = fp.argv[0];
        let fx1 = fp.argv[1];

        match Tag::type_of(mu, fx0) {
            Type::Fixnum => match Tag::type_of(mu, fx1) {
                Type::Fixnum => {
                    fp.value = if Fixnum::as_i64(mu, fx0) < Fixnum::as_i64(mu, fx1) {
                        Tag::t()
                    } else {
                        Tag::nil()
                    };

                    Ok(())
                }
                _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-lt", fx1)),
            },
            _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-lt", fx0)),
        }
    }

    fn mu_fxdiv(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let fx0 = fp.argv[0];
        let fx1 = fp.argv[1];

        match Tag::type_of(mu, fx0) {
            Type::Fixnum => match Tag::type_of(mu, fx1) {
                Type::Fixnum => {
                    let dividend = Fixnum::as_i64(mu, fx0);
                    let divisor = Fixnum::as_i64(mu, fx1);

                    if divisor == 0 {
                        return Err(Exception::raise(
                            mu,
                            Condition::ZeroDivide,
                            "mu:fx-div",
                            fx0,
                        ));
                    }

                    fp.value = Self::as_tag(dividend / divisor);
                    Ok(())
                }
                _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-div", fx1)),
            },
            _ => Err(Exception::raise(mu, Condition::Type, "mu:fx-div", fx0)),
        }
    }

    fn mu_fxand(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let fx0 = fp.argv[0];
        let fx1 = fp.argv[1];

        match Tag::type_of(mu, fx0) {
            Type::Fixnum => match Tag::type_of(mu, fx1) {
                Type::Fixnum => {
                    fp.value = Self::as_tag(Fixnum::as_i64(mu, fx0) & Fixnum::as_i64(mu, fx1));
                    Ok(())
                }
                _ => Err(Exception::raise(mu, Condition::Type, "mu:logand", fx1)),
            },
            _ => Err(Exception::raise(mu, Condition::Type, "mu:logand", fx0)),
        }
    }

    fn mu_fxor(mu: &Mu, fp: &mut Frame) -> exception::Result<()> {
        let fx0 = fp.argv[0];
        let fx1 = fp.argv[1];

        match Tag::type_of(mu, fx0) {
            Type::Fixnum => match Tag::type_of(mu, fx1) {
                Type::Fixnum => {
                    fp.value = Self::as_tag(Fixnum::as_i64(mu, fx0) | Fixnum::as_i64(mu, fx1));
                    Ok(())
                }
                _ => Err(Exception::raise(mu, Condition::Type, "mu:logor", fx1)),
            },
            _ => Err(Exception::raise(mu, Condition::Type, "mu:logor", fx0)),
        }
    }
}

#[cfg(test)]
mod tests {
    use crate::types::fixnum::Fixnum;

    #[test]
    fn as_tag() {
        match Fixnum::as_tag(0) {
            _ => assert_eq!(true, true),
        }
    }
}
